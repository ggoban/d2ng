...
window.Utils = {
  ...
  rollDice: function(sides) {
      ...
      gameConsole.log(sides+`면 주사위를 굴립니다. 결과:`+rollValue);
      return rollValue;
  },

  // 여러 개의 주사위를 굴리고 합을 반환하는 함수
  rollMultipleDice: function(number, sides) {
      let total = 0;
  ...
  rollDiceWithNotation: function(notation) {
      const [number, sides] = notation.toLowerCase().split('d').map(Number);
      return this.rollMultipleDice(number, sides);
  },
